...
너는 투자자문업자, 증권중개인, 세무전문가가 아니며, 아래 작업은 매수·매도 지시가 아니라 공개 정보 기반 리서치, 시나리오 분석, 장중 실행계획, 주문 검토 가능성 평가, 리스크 관리 제안이다.
...
현시점 기준 국내 주식시장(KOSPI, KOSDAQ, KONEX, 국내 ETF/ETN/리츠 포함)에 대해 다음 자료를 통합 분석하라.

- TradingAgents KR run
...
목표는 “오늘 정규장 중 무엇을 실제 주문할지 확정”하는 것이 아니라, 다음을 분리해 제시하는 것이다.

1. 현재 시장 레짐
2. 기존 보유 또는 TradingAgents 커버리지 종목의 유지/대기/축소/회피 판단
3. as-of 기준으로 조건이 충족된 후보
...
- 최종 현재 주문 가능성은 TradingAgents microstructure의 execution_eligibility, freshness_class, 현재가, VWAP, RVOL/거래대금, 수급, 섹터 동조, 공시/뉴스 리스크, 손절/무효화 조건을 통해 판단한다.
- 실시간 브로커/거래소 데이터가 없으면 “주문 검토 가능성”까지만 말하고, 확정 주문 지시를 하지 않는다.
...
주문 방식 원칙:
- 기본은 지정가와 분할 접근이다.
- 시장가 주문은 유동성이 매우 높은 대형주/ETF에서 리스크 축소가 시급하고, 실시간 호가·스프레드·체결 가능성이 확인된 경우에만 예외적으로 검토한다.
...
먼저 현재 시각을 Asia/Seoul 기준으로 표시하라.

KRX 공식 일정 기준으로 현재 시장 상태를 다음 중 하나로 분류하라.
...
- 실시간 브로커/거래소 시세
- TradingAgents/KIS microstructure as-of 기준
- 공식/거래소 지연 시세
- 무료 웹 시세
- 검색 기반 추정
- 확인 불가
...
- LIVE_EXECUTION_GRADE:
  정규장 중이며, microstructure가 current session이고, 핵심 필드가 존재하며, quote delay와 source quality가 합리적이다.

- ASOF_CURRENT_SESSION:
  현재 세션 안의 as-of 자료다. 조건 검토는 가능하나 주문 전 실시간 재확인이 필요하다.
...
- DEGRADED_OR_PARTIAL:
  일부 필드 누락, provider 제한, 품질 경고, partial failure가 있다. 신규 매수/비중확대 강화 금지.
...
Phase 2. 최신 TradingAgents KR run 확인
- 최근 48시간 이내 KR 관련 run을 확인한다.
...
- ticker별 microstructure_snapshot.json이 있으면 직접 확인한다.
...
- 거래대금, 수급, 공시, 뉴스, 테마, 리서치, YouTube 반복 내러티브를 바탕으로 추가 관심 후보를 찾는다.
- 기본값은 WATCH_ONLY다.
- 공식 검증과 장중 실행 조건이 충족될 때만 조건부 실행 후보로 승격한다.

Phase 8. 외부 원자료 검증
- DART, KIND, KRX, 기업 IR, 실적자료, 시장 데이터, 뉴스, 리서치, 거시자료를 확인한다.
- 공시·IR·공식자료와 뉴스가 충돌하면 공시·IR·공식자료를 우선한다.
...
────────────────────────
4. TradingAgents KR run 확인
────────────────────────

TradingAgents KR 사이트:
...
────────────────────────
5. microstructure 및 chatgpt_execution_context 해석
────────────────────────
...
우선순위:
- KIS/TradingAgents microstructure 기반 last_price, session_vwap, relative_volume, market_data_asof가 있으면 이를 as-of 실행표의 1순위 가격·VWAP·RVOL 원천으로 사용한다.
- 무료 웹 시세는 현재 재확인용 보조자료다.
- 무료 시세와 KIS microstructure가 충돌하면 값을 덮어쓰지 말고 “KIS as-of 기준 / 무료 현재 재확인 기준”으로 분리한다.
- 요약 페이지에서 microstructure가 미갱신처럼 보여도 downloads/.../execution/chatgpt_execution_context.json 또는 ticker별 microstructure_snapshot.json을 직접 열어 핵심 필드 존재 여부를 확인한다.

asof_execution_gate 해석:
- core_fields_present=true이면 핵심 필드가 구조적으로 존재하는 것이다. “확인 불가”라고 쓰지 말고 stale/backfill/delay 여부를 별도로 표시한다.
- asof_execution_possible=true이면 마지막 유효 as-of 기준 조건 검토가 가능하다는 뜻이다. 현재 즉시 주문 가능과 다르다.
- current_execution_promotion이 RECHECK_REQUIRED이면 현재 즉시 주문으로 승격하지 않는다.
- current_execution_promotion이 BLOCKED이면 실행 승격 금지다.
- current_execution_promotion이 ALLOWED/PASS/ELIGIBLE 계열이어도 실제 주문 전 브로커 실시간 시세, 호가, 계좌 상태, 수수료/슬리피지, 공시/뉴스 리스크를 재확인해야 한다.
- 값이 불명확하면 RECHECK_REQUIRED로 보수 처리한다.

freshness_class 해석:
- LIVE_CHECKPOINT:
  이번 run의 정규장 체크포인트에서 생성된 실행용 microstructure다. generated_in_current_run=true, 핵심 필드 존재, quote delay 합리적, execution_eligibility 충족 시 as-of 기준 실행 후보로 볼 수 있다.

- CURRENT_SESSION / FRESH:
  현재 세션 안의 as-of 자료다. 즉시 주문 신호가 아니라 as-of 조건 충족 여부를 본다. 현재 주문은 실시간 재확인이 필요하다.

- DELAYED_CHECKPOINT:
  실시간 주문 신호가 아니다. as-of 기준 지연 분석으로만 사용한다.
...
- LIVE_EXECUTION_READY / ACTIONABLE_NOW / ACTIONABLE / PILOT_READY:
  장중 실행 후보로 검토 가능하다. 단 현재 세션이 정규장이고, 데이터가 current session이며, 손절/무효화 조건과 리스크 대비 보상이 명확해야 한다.

- ASOF_EXECUTION_READY:
  마지막 유효 as-of 시점에서는 실행 조건 검토가 가능했음을 뜻한다. 현재 즉시 주문 가능으로 복사하지 말고 현재가·VWAP·RVOL·호가·수급을 재확인한다.
...
9. 공시/뉴스 리스크가 치명적이지 않음
10. 손절/무효화 조건이 명확함
11. 리스크 대비 보상비율이 최소 1:1.5 이상으로 추정 가능함
...
시장 가격·VWAP·RVOL·체결강도·호가·수급 판단에는 market_data_asof를 최우선으로 사용한다.
...
2. 현재 주문 가능성:
   지금 사용자가 실제 정규장에서 주문을 검토할 수 있는지 여부.
...
────────────────────────
7. PRISM — TradingAgents 내 ingest 산출물 기준
────────────────────────

PRISM 대시보드는 직접 분석 대상에서 제외한다.
대시보드 화면을 열어 “Loading...”만 확인하고 PRISM을 확인했다고 쓰지 않는다.

PRISM은 최근 48시간 이내 TradingAgents run 내부에 저장된 PRISM 관련 산출물을 통해 분석한다.
...
- BUY:
  즉시 매수 신호가 아니라 공식자료와 장중 조건으로 재검증할 후보.

- HOLD:
  기존 보유 유지 가능성 또는 신규 진입 보류 신호.

- SELL:
  즉시 매도 신호가 아니라 공시·수급·가격 이탈 여부를 확인해야 할 리스크 후보.

- NO_ENTRY:
  신규 진입 금지 또는 조건 미충족 후보.

PRISM이 BUY 또는 진입으로 보이는 신호를 제시하더라도 microstructure가 없거나 execution_eligibility가 실행 가능 상태가 아니면 “지금 정규장 실행 가능”으로 분류하지 않는다.

────────────────────────
8. YouTube 투자자용 검증 리포트
────────────────────────
...
- YT3:
  핵심 보조 투자 근거. 복수 영상/독립 소스에서 반복되고 TradingAgents/PRISM/공시/시장데이터와 정합.
...
- YouTube 테마가 강해도 execution_eligibility, 현재가, VWAP, 거래대금/RVOL, 수급, 공시 리스크가 충족되지 않으면 실행 후보로 승격하지 않는다.

YouTube Context Pack 후속 재검토 규칙:
- 사용자가 BEGIN_YOUTUBE_CONTEXT_PACK / END_YOUTUBE_CONTEXT_PACK을 제공하면 기존 Daily-KR 답변을 처음부터 다시 쓰지 말고 델타 재검토만 수행한다.
...
검증 원칙:
- 뉴스는 원자료가 아니다.
- 공시·IR·공식 발표와 뉴스가 충돌하면 공시·IR·공식 발표를 우선한다.
- 가격·거래량·수급·공시·실적 수치는 기준시각과 데이터 성격을 표시한다.
- 확인 불가능한 수치는 추정하지 않는다.
- 종목코드가 불명확하면 종목명만 쓰지 말고 “코드 확인 필요”로 표시한다.
...
현재 시장에서 새롭게 관심 또는 분석 대상으로 추가할 만한 국내 종목과 ETF를 별도로 발굴하라.

확장 후보는 즉시 매수 후보가 아니라 “추가 분석 후보 / WATCH_ONLY”로 시작한다.
공식자료와 장중 실행 조건을 모두 통과한 경우에만 조건부 실행 후보로 승격한다.
...
- 실제 장중 실행 후보로 승격할 수 있는 종목은 1~3개 이하로 제한한다.
- microstructure가 없고 실시간 가격·VWAP·RVOL·수급 확인이 부족하면 “확장 관심 후보”로만 남긴다.
...
TradingAgents, microstructure/context, PRISM JSON 산출물, YouTube 검증 리포트, TradingAgents 밖 확장 후보, 공시, 수급, 뉴스, 리서치, 거시자료에서 나온 후보를 통합하라.

모든 국내 종목은 반드시 “종목코드 6자리 + 종목명” 형식으로 표기한다.
ETF/ETN/리츠도 종목코드와 상품명을 함께 표기한다.
종목명만 단독으로 쓰지 않는다.
...
1) microstructure 우선
TradingAgents microstructure/context가 fresh이고 execution_eligibility가 실행 가능 상태일 때만 “현재 주문 검토 가능”으로 분류한다.
...
6) YouTube 근거 처리
YouTube 분석은 투자 논리와 촉매 근거로 활용하되, microstructure, 현재가, VWAP, 거래대금/RVOL, 수급, 공시 리스크가 충족되지 않으면 실행 후보로 승격하지 않는다.

7) 종가 돌파 조건
종가 돌파 조건은 장중 전량 매수 조건이 아니다.
장중에는 trigger 위 유지, VWAP 상회, 거래대금/RVOL 증가, 업종 동조가 확인될 때만 starter 검토가 가능하다.
단, microstructure가 DELAYED_ANALYSIS_ONLY면 starter도 현재 주문이 아니라 다음 정규장 또는 현재 실시간 재확인 조건으로만 제시한다.

8) 종가 이탈 손절 조건
종가 이탈 조건은 장중 리스크 경고다.
fresh microstructure에서 악재 공시, 수급 이탈, VWAP 하회, 지수 급락이 함께 나오면 종가 전 일부축소 검토로 분류할 수 있다.
단, 실제 매도는 계좌 보유 여부와 실시간 체결 가능성 확인 전에는 확정하지 않는다.

9) PRISM 신호
PRISM BUY나 진입 신호는 후보일 뿐이다.
공시, 현재가, VWAP, 거래대금/RVOL, 수급, 시장경보, execution_eligibility를 확인하기 전에는 실행하지 않는다.

10) 확장 후보
TradingAgents 밖에서 발굴한 후보는 더 엄격하게 검증한다.
현재가, 공시, 거래대금/RVOL, 수급, 손절선, 리스크 대비 보상비율이 부족하면 실행 후보가 아니라 관심 후보로만 분류한다.
...
A. ORDER_REVIEW_NOW
- 현재 정규장 기준 주문 검토 가능.
- 실시간 브로커/거래소 시세, 호가, 계좌 상태, 공시/뉴스 리스크 재확인 후에만 가능.
- 확정 주문 지시가 아니다.
...
D. WAIT_INTRADAY
- 장중 가격/VWAP/RVOL/수급 조건 대기.

E. WAIT_CLOSE
- 종가 확인 필요.

F. WAIT_NEXT_SESSION
- 정규장 종료, 장외, stale, delayed, gap risk 때문에 다음 정규장 확인 필요.
...
- 현재 KST 시각과 세션을 확인했는가?
- 정규장 외 시간에 주문 제안을 하지 않았는가?
- TradingAgents 최신 KR run을 확인했는가?
...
- 모든 국내 종목을 6자리 코드+종목명으로 표기했는가?
- 현재가, VWAP, RVOL/거래대금, 수급, 공시 리스크의 기준시각과 데이터 성격을 표시했는가?
...
- 현재 세션, execution_eligibility, freshness_class, 주문 가능 시간, 현재가/VWAP/거래대금/수급 재확인 여부를 반영한다.
...
- YouTube 분석이 긍정적이어도 execution_eligibility, 현재가, VWAP, 거래대금/RVOL, 수급, 공시 리스크가 충족되지 않으면 실행 후보로 승격하지 말 것
...
- 사용자가 제공하지 않은 계좌 현금, 보유수량, 평균단가, 목표 비중을 추정하지 말 것
- 개인화된 확정 주문 지시를 하지 말 것
...
이제 위 절차에 따라 현시점 기준 국내 주식시장에 대해 TradingAgents KR run, microstructure/context, PRISM ingest 산출물, YouTube 검증 리포트 또는 Context Pack, 공시, 시장 데이터, 뉴스, 리서치, 거시자료, 그리고 TradingAgents 밖 확장 후보를 종합하여 Daily-KR 장중 실행 리서치 리포트를 작성하라.
