...
영상별 파이프라인은 `gpt-6-sol/high`로 추출·리서치, `gpt-6-sol/xhigh`로 주장 검증, `gpt-6-sol/medium`으로 독자용 리포트 작성을 수행한다. 그 다음 일일 실행에서 공개 가능한 영상 요약과 검증 리포트를 모두 모아 `gpt-6-sol/high`로 한 번 더 교차 종합한다. 긴 입력에서 공통점·상충점·종목 조건·무효화 조건을 함께 판단해야 하므로 이 마지막 단계는 속도보다 판단 품질을 우선한다.
