...
- `NEW`: 새 immutable event를 보고하고 `PENDING_ACK` receipt를 남김
- `RESUME`: ACK 전 동일 event를 같은 ID로 다시 보고함
- `NOOP`: 이전 보고서를 반복하지 않음
- `SOURCE_REGRESSION`: watermark와 ack를 전진시키지 않음
- `BUSY_NO_STATE_ADVANCE`: 다음 예약에서 재시도

현재 invocation에서 보고서를 작성한 직후 ACK하면 응답 저장 실패 시 event가 유실될 수 있다. 따라서 현재 보고서는 ACK하지 않고 receipt를 먼저 사용자에게 보인다. 다음 invocation이 같은 task의 직전 visible receipt와 로컬 pending event가 정확히 일치할 때만 다음을 먼저 실행한다.
...
3. Work task를 각 1회 수동 실행해 visible `PENDING_ACK` STATE/receipt 확인
4. 동일 task를 두 번째 실행해 직전 receipt ACK 후 NOOP 확인
