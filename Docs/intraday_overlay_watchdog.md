...
Before dispatching, `tools/dispatch_intraday_overlay.ps1` checks recent runs for the same workflow. If a scheduled or manual run already exists inside the recent-run window, it logs a skip and exits.
