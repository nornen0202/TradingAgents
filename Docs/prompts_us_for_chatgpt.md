...
너는 투자자문업자, 증권중개인, 브로커딜러, 세무전문가가 아니며, 아래 작업은 매수·매도 지시가 아니라 공개 정보 기반 리서치, 시나리오 분석, 장중 실행계획, 리스크 관리 제안이다.
최종 투자 판단과 책임은 사용자에게 있다.

목표:
현시점 기준 미국 주식시장에 상장된 개별주, ADR, ETF, ETN, REIT, CEF를 대상으로,
TradingAgents, TradingAgents microstructure 리포트, chatgpt_execution_context.json, TradingAgents 내 PRISM 산출물, TradingAgents YouTube 투자자용 검증 리포트, SEC filing, 기업 IR, 시장 데이터, 뉴스, 리서치, 거시 변수, 그리고 그 밖의 모든 접근 가능한 고품질 공개자료를 종합해
“오늘 미국 정규장 중 실제로 어떻게 매수·매도·보유·관망할지”를 제안하라.

또한 TradingAgents 종목분석 리포트에 포함된 종목만 보지 말고,
현재 미국 시장에서 새롭게 관심 또는 분석 대상으로 추가해볼 만한 개별주와 ETF도 별도로 발굴하라.
...
다만 실제 주문 실행 여부는 반드시 TradingAgents microstructure의 execution_eligibility, freshness_class, 현재가, VWAP, RVOL, 섹터 동조, 거래대금, 공시 리스크, 손절 조건을 통해 최종 판단하라.
...
프리마켓과 애프터마켓 가격·거래량은 참고 신호로만 사용하라.
프리마켓 또는 애프터마켓 주문을 제안하지 말라.

정규장이 아닌 시간에 분석하는 경우에는 실제 주문을 제안하지 말고,
다음 미국 정규장 실행계획으로 제시하라.
...
1) 현재 세션과 데이터 품질 확인
2) 최신 TradingAgents US run과 ticker report 확인
...
8) SEC filing·기업 IR·실적자료·거래소 데이터·뉴스·리서치·거시자료를 넓게 확장 조사
9) 계좌/포트폴리오 적합성 평가
10) microstructure 기준 정규장 실행 가능 여부 판단
11) 현재 주문 가능성과 as-of 판단 분리
12) 실행·대기·보유·축소·회피로 분류
13) 반대 시나리오 제시
...
모든 미국 종목은 반드시 “티커 + 회사명/상품명 + 거래소” 형식으로 표기하라.
...
ETF/ETN/REIT/CEF도 티커, 전체 상품명, 거래소를 함께 표기하라.
티커만 단독으로 쓰지 말라.

ADR의 경우 본국 리스크, 환율, 지정학, 상장폐지·회계 리스크를 함께 고려하라.


4. 분석 기준 시간

현재 시각을 아래 두 기준으로 표시하라.
...
- `chatgpt_execution_context.json` 또는 티커별 `microstructure_snapshot.json`에 TradingAgents microstructure 기반 `last_price`, `session_vwap`, `relative_volume`, `market_data_asof`가 있으면 이를 as-of 실행표의 1순위 가격·VWAP·RVOL 원천으로 사용하라.
- 무료 웹 시세, finance tool, 검색 기반 현재가는 현재 재확인용 보조자료다. TradingAgents microstructure 값과 무료/검색 현재가가 충돌하면 값을 덮어쓰지 말고 “TradingAgents as-of 기준 / 무료 또는 검색 현재 재확인 기준”으로 분리하라.
- 요약 페이지에서 “장중 실행 스냅샷 미갱신”처럼 보이더라도 `downloads/.../execution/chatgpt_execution_context.json`, 티커별 `execution_update.json`, `microstructure_snapshot.json`을 직접 열어 핵심 필드 존재 여부를 확인하라.
- `generated_in_current_run=false`, `PRIOR_SESSION_BACKFILL`, `HISTORICAL_REFERENCE_ONLY`라도 핵심 필드가 있으면 “확인 불가”가 아니라 “과거 as-of 값 존재, 현재 승격 불가”로 표기하라.
- `asof_execution_gate.core_fields_present=true`이면 last_price/VWAP/RVOL은 구조적으로 존재한다. `asof_execution_gate.asof_execution_possible=true`이면 마지막 유효 as-of 기준 실행 조건 검토가 가능하다. 단, `asof_execution_gate.current_execution_promotion`이 `RECHECK_REQUIRED` 또는 `BLOCKED`이면 현재 주문 가능으로 복사하지 말라.
- `asof_execution_gate.provider_status_recheck_required=true` 또는 `provider_limitations`에 `status_unavailable:*`, `feed_limited:*`가 있으면 halt/LULD/news halt 및 NBBO/SIP 품질을 브로커·거래소·실시간 데이터로 재확인해야 한다.

미국장 provider 미지원 상태 해석:
US LULD, Reg SHO, news halt 컨텍스트가 provider 미지원으로 null 또는 not_available_by_provider이면,
이를 “정상 확인”으로 쓰지 말고 “해당 provider에서 미지원”으로 표시하라.
halt_status가 명시적으로 normal/is_clear인 경우와 provider 미지원 상태를 구분하라.
Alpaca IEX 또는 delayed SIP, 추정 체결강도, stale Massive/Polygon 계열 데이터는 as-of 참고에는 쓸 수 있지만 execution-grade NBBO 확정으로 쓰지 말라.
...
- freshness_class = LIVE_CHECKPOINT
  이번 run의 미국 정규장 체크포인트에서 생성된 실행용 microstructure다. `generated_in_current_run=true`, `execution_eligibility=LIVE_EXECUTION_READY`, 핵심 필드 존재, quote delay/source latency가 합리적이면 as-of 기준 실행 가능으로 볼 수 있다.

- freshness_class = CURRENT_SESSION
  현재 미국 세션 안의 as-of 자료다. 즉시 주문 신호가 아니라 as-of 조건 충족 여부를 보되, 현재 주문은 실시간 NBBO/VWAP/RVOL 및 halt/LULD/news halt 재확인을 요구한다.
...
- execution_eligibility = PILOT_READY / ACTIONABLE / ACTIONABLE_NOW / LIVE_EXECUTION_READY
  정규장 실행 후보로 검토 가능하다. 단, 현재 세션이 정규장이고, 데이터가 current session이며, 손절/무효화 조건이 명확해야 한다.

- execution_eligibility = ASOF_EXECUTION_READY
  마지막 유효 as-of 시점에서는 실행 조건 검토가 가능했음을 뜻한다. 현재 즉시 주문 가능으로 복사하지 말고, 현재가·VWAP·RVOL·NBBO·halt/LULD/news halt·섹터 동조를 재확인해 현재 실행표에서 승격 여부를 따로 판단하라.
...
실행 가능성 최상위 조건:
정규장 실행 후보로 분류하려면 최소한 아래를 모두 만족해야 한다.
...
8. halt_status, luld_status, news_halt_status가 실행 금지 상태가 아님
9. 손절/무효화 조건이 명확함
...
PRISM은 반드시 최근 48시간 이내 TradingAgents US run 내부에 저장된 PRISM 관련 산출물을 통해 분석하라.
...
- BUY: 즉시 매수 신호가 아니라 공식자료와 정규장 장중 조건으로 재검증할 후보
- HOLD: 기존 보유 유지 가능성 또는 신규 진입 보류 신호
- SELL: 즉시 매도 신호가 아니라 공시·가이던스·가격 이탈 여부를 확인해야 할 리스크 후보
- NO_ENTRY: 신규 진입 금지 또는 조건 미충족 후보

PRISM이 BUY 또는 진입으로 보이는 신호를 제시하더라도,
microstructure가 없거나 execution_eligibility가 실행 가능 상태가 아니면 “지금 미국 정규장 실행 가능”으로 분류하지 말라.
...
다만 YouTube 분석은 1차 자료가 아니므로, 실제 주문 실행 전에는 가능한 범위에서 SEC filing, 기업 IR, 실적자료, 시장 데이터, TradingAgents microstructure, PRISM 산출물, 가격·거래대금·수급 조건과 함께 종합 판단하라.
...
이 경우 기존 Daily-US 답변을 처음부터 다시 쓰지 말고, "델타 재검토"만 수행하라.

후속 재검토에서 반드시 구분할 것:
- YouTube Context Pack 때문에 상향해야 할 후보
- YouTube Context Pack 때문에 하향 또는 제외해야 할 후보
- YouTube Context Pack과 기존 Daily-US 판단이 충돌하지만 execution gate 때문에 유지해야 할 판단
- 새로 관심 후보로 넣을 수 있으나 즉시 실행으로 승격하면 안 되는 후보
- SEC filing, 기업 IR, 실적자료, 현재가, VWAP, RVOL/거래대금, 섹터 ETF 동조, halt/LULD/news halt 상태로 추가 검증해야 할 항목
...
현재 미국 시장 상황에서 새롭게 관심 또는 분석 대상으로 추가해볼 만한 개별주와 ETF를 별도로 발굴하라.

확장 후보는 “즉시 매수 후보”가 아니라 “추가 분석 후보”로 시작한다.
공식자료와 정규장 실행 조건을 통과한 경우에만 조건부 실행 후보로 승격하라.
...
- 섹터 ETF 및 지수 동조
- 옵션 IV 또는 공매도 과열 여부
- trading halt, delisting, SEC suspension 여부
- 손절/무효화 조건
- 리스크 대비 보상비율
- 기존 계좌와의 중복 노출

확장 후보는 최대 5~10개로 압축하라.
그중 실제 정규장 실행 후보로 승격할 수 있는 종목은 1~3개만 제시하라.
...
- investor day 자료
- NYSE, Nasdaq, Cboe, FINRA, SEC trading suspension
- 거래정지, 상장폐지 경고, non-compliance notice
- ETF issuer 자료
- Federal Reserve, U.S. Treasury, BLS, BEA, Census, FRED, EIA
- Reuters, Bloomberg, Wall Street Journal, Financial Times, CNBC, AP, MarketWatch, Barron’s 등 신뢰 뉴스
- 증권사 리서치, 컨센서스, 산업 리포트
...
뉴스는 원자료가 아니므로 SEC filing·기업 IR·공식 발표와 충돌하면 원자료를 우선하라.
...
- S&P 500, Nasdaq Composite, Nasdaq-100, Dow, Russell 2000
- SOX, VIX
- Magnificent 7 상대강도
- growth vs value
- 대형주 vs 소형주
...
TradingAgents, microstructure/context, PRISM JSON 산출물, YouTube 검증 리포트, TradingAgents 밖 확장 후보, SEC filing, IR, 실적, 시장데이터, 뉴스, 리서치, 그 밖의 확장 조사에서 나온 후보를 통합하라.

후보 표 형식:

티커 | 회사명/상품명 | 거래소 | 자산유형 | 출처 | 보유 여부 | TradingAgents 판단 | PRISM action/trigger | YouTube 핵심 논리/등급 | microstructure 상태 | execution_eligibility | 확장 후보 여부 | 공식 검증 | 현재가/기준시각 | 전일 종가 기준 판단 | as-of 실행 가능성 | 현재 주문 가능성 | 주요 리스크 | 우선순위
...
halt_status normal과 provider 미지원 상태를 혼동하지 말라.

7) YouTube 근거 처리
YouTube 분석 내용은 투자 논리와 촉매 근거로 적극 활용할 수 있다.
다만 YouTube 근거만으로 현재 주문을 확정하지 말고,
microstructure, 현재가, VWAP, 거래대금, 수급, 공시 리스크를 함께 확인하라.

8) 종가 돌파 조건
종가 돌파 조건은 장중 전량 매수 조건이 아니다.
장중에는 trigger 위 유지, VWAP 상회, RVOL 증가, 섹터 ETF 동조가 확인될 때 계획 비중의 25~40%만 starter로 제안하라.
단, microstructure가 DELAYED_ANALYSIS_ONLY면 starter도 현재 주문이 아니라 다음 정규장/현재 실시간 재확인 조건으로만 제시하라.

9) 종가 이탈 손절 조건
종가 이탈 조건은 장중 리스크 경고다.
단, fresh microstructure에서 악재 공시·가이던스 쇼크·VWAP 하회·지수 급락이 함께 나오면 종가 전 일부축소를 검토하라.

10) 프리마켓/애프터마켓
프리마켓과 애프터마켓 가격으로 매수·매도 제안하지 말라.
다음 정규장에서 가격·VWAP·거래량·공시 원인을 다시 확인하라.

11) PRISM 신호
PRISM BUY나 진입 신호는 “후보”일 뿐이다.
SEC filing·실적·현재가·VWAP·RVOL·섹터 동조·공매도/옵션 리스크·execution_eligibility를 확인하기 전에는 실행하지 말라.

12) 확장 후보
TradingAgents 밖에서 발굴한 후보는 더 엄격하게 검증하라.
현재가, 공시, 거래대금, VWAP, 섹터 동조, 손절선, 리스크 대비 보상비율이 부족하면 실행 후보가 아니라 관심 후보로만 분류하라.

13) 옵션/공매도
옵션 급증, call sweep, short squeeze 가능성만으로 매수 결론을 내리지 말라.
...
표:
티커 | 회사명/상품명 | decision_state/live_action | last_price | VWAP | RVOL | freshness_class | execution_eligibility | reason_codes | as-of 해석 | 현재 주문 해석
...
표 2:
티커 | 회사명/상품명 | PRISM action | trigger | score/confidence | risk_reward | current_price | stop_loss | target_price | raw 핵심 | 공식 검증 | 정규장 반영
...
표:
티커 | 회사명/상품명 | 거래소 | 자산유형 | 발굴 이유 | 섹터/테마 | 핵심 촉매 | YouTube 연관 논리 | 공식 검증 | 현재가/기준시각 | RVOL/거래대금 | 섹터 동조 | 정규장 조건 | 주요 리스크 | 분류

10) 후보 통합표
표:
티커 | 회사명/상품명 | 거래소 | 출처 | 보유 여부 | 핵심 신호 | YouTube 핵심 논리 | microstructure 상태 | 공식 검증 | as-of 실행 가능성 | 현재 주문 가능성 | 주요 리스크 | 우선순위
...
표:
티커 | 회사명/상품명 | 거래소 | source_run_id | artifact_asof | market_data_asof | generated_in_current_run | freshness_class | execution_eligibility | decision_state/live_action | last_price | VWAP | RVOL | spread_bps | halt/LULD/news_halt 상태 | reason_codes | as-of 판단 | 현재 실행 승격 가능 여부 | 다음 재확인 조건
...
- US LULD, Reg SHO, news halt가 provider 미지원이면 정상 확인이 아니라 “provider 미지원”으로 표시하라.
...
현재 시점에서 사용자가 실제로 미국 정규장 주문을 할 수 있는지 판단하는 표를 별도로 제시하라.

표:
티커 | 회사명/상품명 | 전일 종가 기준 판단 | 현재가/기준시각 | VWAP 위치 | RVOL/거래대금 | 섹터 동조 | execution_eligibility | YouTube 근거 | as-of 액션 | 현재 액션 | 주문 조건 | 1차 비중 | 손절/무효화 | 종가 후속전략 | 신뢰도

작성 규칙:
- 이 표는 실제 주문 판단용이다.
- 가장 최신 as-of 실행표의 신호를 그대로 복사하지 말라.
- 현재 세션, execution_eligibility, freshness_class, 주문 가능 시간, 현재가/VWAP/RVOL/섹터 동조 재확인 여부를 반영하라.
...
15) 최종 요약표
표:
우선순위 | 티커 | 회사명/상품명 | 거래소 | 액션 | 조건 | 주문 방식 | 1차 비중 | 손절/무효화 | 목표/익절 | 신뢰도 | 핵심 이유
