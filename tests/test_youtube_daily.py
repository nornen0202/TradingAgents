...
class YouTubeDailyTests(unittest.TestCase):
    ...
    def test_github_actions_workflow_schedule_and_pages_artifact(self):
        ...
        self.assertIn("Target: 20:00 KST every day", workflow)
        self.assertIn("17 11 * * *", workflow)
        self.assertIn("47 11 * * *", workflow)
        self.assertIn("17 12 * * *", workflow)
        self.assertIn("47 12 * * *", workflow)
        self.assertIn("replace(hour=19, minute=0", workflow)
        self.assertIn("including weekends", workflow)
        self.assertIn("schedule_gate", workflow)
        self.assertIn("actions: read", workflow)
        self.assertIn("cancel-in-progress: false", workflow)
        self.assertNotIn("0 13 * * *", workflow)
        self.assertNotIn("17 13 * * *", workflow)
        self.assertNotIn("47 13 * * *", workflow)
