...
""",
            ...
            self.assertTrue((private_dir / "account_review.md").exists())
            account_review = json.loads((private_dir / "account_review.json").read_text(encoding="utf-8"))
            self.assertFalse(account_review["order_authorized"])
            self.assertIsNone(account_review["combined_nav_krw"])
