...
def test_youtube_packet_preserves_evidence_ids_for_claim_mapping(tmp_path: Path):
        ...
        json.dumps(
            {
                ...
                "strategy_trust_status": "USER_VERIFIED_PRIMARY",
                "assumed_verified_for_strategy": True,
    ...
    assert event["trusted_primary"] is True
    assert event["strategy_trust_status"] == "USER_VERIFIED_PRIMARY"
...
def test_external_context_ranks_ticker_relevance_and_declares_balanced_policy():
    ...
    assert policy["trusted_primary_evidence_weight"] == "HIGH"
    assert policy["trusted_primary_assumed_verified"] is True
